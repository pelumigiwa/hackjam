...
define(function() {
    ...
    function Registration(req, res, next) {
        try {
            ...
            validation.Validator(req.body, validationRule, {}, (err, status) => {
                if (!status) {
                    ...
                    var password = jswt.HarshPassword(req.body.password),
                        jsnReq = { firstName: req.body.firstName, lastName: req.body.lastName, emailAddress: req.body.emailAddress, phoneNumber: req.body.phoneNumber, password: password, channel: req.body.channel },
                        jsnDta = JSON.stringify(jsnReq);

                    logger.info('/registration', jsnDta);

                    let regUser = new regDao(req.body.firstName, req.body.lastName, req.body.emailAddress, req.body.phoneNumber, password, req.body.channel, jsnDta);
                            ...
                            switch (data.status.toLowerCase()) {
                                case "ok":
                                    var verifyLink = jswt.JwtSign(req.body.emailAddress),
                                        options = { header: 'Email Verification', email: req.body.emailAddress, name: data.CustomerFirstName, link: verifyLink };
            ...
            dispatch.DispatchErrorMessage(res, 'application error in Registration()..');
        }
    }
    ...
    function RegistrationVerification(req, res, next) {
        try {
                ...
                if (!status) {
                    ...
                    jswt.JwtVerifySign(token, (err, resp) => {
                        if (err) {
                            ...
                            let mail_auth = { token: resp.userId, channel: channel, jsonData: jsnDta };
                            bseDao.Query(regDao.GetVerifyEmailSQL(mail_auth), (err, resp) => {
                                if (err) {
                                    ...
                                    var data = JSON.parse(resp);
                                    dispatch.SendGenricMessage(res, data);
                                    logger.info('/registrationVerification', JSON.stringify(data));
                                }
                            });
                        }
                    })
            ...
            dispatch.DispatchErrorMessage(res, 'application error in RegistrationVerification()..');
        }
    }
